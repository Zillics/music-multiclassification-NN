...
def dim_reduction(X,variance_threshold,correlation_threshold,print_status=False):    
    ...
    X_norm = np.delete(X,remove_indices,axis=1) # Delete in order to avoid division by 0 in normalization
    X_norm = (X_norm - X_norm.min(axis=0))/(X_norm.max(axis=0) - X_norm.min(axis=0))
    i = remove_indices[0]
...
def main():
	...
	evaluate_model(model,X_train,Y_train,X_test,Y_test,epochs=20000,batch_size=32,filename="models/model_12/model_12_regul_09_1024_1024_64")
